    ...
    cols.current = columns
      ...
      .map((col) => {
        ...
        if (item?.filter) {
          ...
          switch (item?.filter?.type) {
            case ETableFilterType.radio:
              ...
              item = {
                ...item,
                ...getColumnSearchRadio(item.filter.list as CheckboxOptionType[], col!.name!, item.filter.get),
              };
              break;
        ...
        if (item?.sorter && sort && sort[col!.name!])
          item.defaultSortOrder = sort[col!.name!] === 'asc' ? 'ascend' : sort[col!.name!] === 'desc' ? 'descend' : '';
        if (item && !item?.onCell)
          item.onCell = (record) => ({
