...
class AddressTest extends TestCase
{
  ...
  private function base(ERole $eRole, $permissions = []): void
  {
    ...
    if ($eRole !== ERole::USER) {
      ...
      $res = $this->put('/api/addresses/provinces/' . $res['data'][0]['code'], ['is_disable' => true])->assertStatus($eRole !== ERole::USER ? 200 : 403);
    ...
    $res = $this->put('/api/addresses/districts/' . $district['code'], ['is_disable' => true])->assertStatus($eRole !== ERole::USER ? 200 : 403);
    if ($eRole !== ERole::USER)
      $this->assertTrue($res['data'][Str::camel('is_disable')]);
    ...
    if ($eRole !== ERole::USER)
      $this->assertFalse($res['data'][Str::camel('is_disable')]);
    $res = $this->put('/api/addresses/wards/' . $ward['code'], ['is_disable' => true])->assertStatus($eRole !== ERole::USER ? 200 : 403);
    if ($eRole !== ERole::USER)
      $this->assertTrue($res['data'][Str::camel('is_disable')]);
